...
async def test_upload_fail_auth(
    ...
    payload = JWTPayload(
        ...
        sub="uploader_id", role=Role.UPLOADER, exp=datetime(2000, 1, 1), username="you", email="shall@not.pass"
    )
    files = {"file": text_file.open(mode="rb")}
    response = await test_client.post(
        "/api/files/upload",
        files=files,
        headers={"Authorization": f"Bearer {jwt.encode(payload.dict(), key=settings.SECRET_KEY, algorithm=settings.JWT_ALGORITHM)}"}
    )
...
async def test_upload_as_admin(
    test_client: AsyncClient, test_db: MockDatabase, text_file: Path, admin_token_header: str
):
    files = {"file": text_file.open(mode="rb")}
    response = await test_client.post(
        "/api/files/upload",
...
async def test_upload_to_others_storage_as_admin(
    ...
    # check there is an entry with the uploaded filename
    assert await test_db.client["file_service"]["fs.files"].find_one({"filename": text_file.name, "metadata": {"user_id": "some_id"}})
...
async def test_upload_as_uploader(
    test_client: AsyncClient, test_db: MockDatabase, text_file: Path, uploader_token_header: str
):
    files = {"file": text_file.open(mode="rb")}
    response = await test_client.post(
        "/api/files/upload",
...
async def test_upload_as_viewer(
    test_client: AsyncClient, test_db: MockDatabase, text_file: Path, viewer_token_header: str
):
    files = {"file": text_file.open(mode="rb")}
    response = await test_client.post(
        "/api/files/upload",
...
async def test_download_as_admin(
    ...
    response = await test_client.get("/api/files/download", params={"filename": "text.txt"}, headers=admin_token_header)
...
async def test_download_from_others_storage_as_admin(
    ...
    response = await test_client.get(
        "/api/files/download", params={"filename": "text.txt", "user_id": "some_id"}, headers=admin_token_header
    )
    # check downloaded content matches
    with text_file.open("rb") as f:
        assert f.read() == response.content
...
async def test_download_as_uploader(
    ...
    response = await test_client.get(
        "/api/files/download", params={"filename": "text.txt"}, headers=uploader_token_header
    )
    # check downloaded content matches
    with text_file.open("rb") as f:
        assert f.read() == response.content
...
async def test_download_from_others_storage_as_uploader(
    ...
    with text_file.open("rb") as f:
        await test_db.grid_client.upload_from_stream(filename="text.txt", source=f, metadata={"user_id": "some_id"})
    response = await test_client.get(
        "/api/files/download", params={"filename": "text.txt", "user_id": "some_id"}, headers=uploader_token_header
    )
...
async def test_download_as_viewer(
    ...
    response = await test_client.get(
        "/api/files/download", params={"filename": "text.txt"}, headers=viewer_token_header
    )
    assert response.status_code == status.HTTP_403_FORBIDDEN
...
async def test_read_file_info_list_as_admin(
    test_client: AsyncClient, test_db: MockDatabase, text_file: Path, image_file: Path, audio_file: Path, admin_token_header: str
):
    with text_file.open("rb") as f:
        await test_db.grid_client.upload_from_stream(filename=text_file.name, source=f, metadata={"user_id": "admin_id"})
    with image_file.open("rb") as f:
        await test_db.grid_client.upload_from_stream(filename=image_file.name, source=f, metadata={"user_id": "admin_id"})
    with audio_file.open("rb") as f:
        await test_db.grid_client.upload_from_stream(filename=audio_file.name, source=f, metadata={"user_id": "admin_id"})
    response = await test_client.get(
        "/api/files/list",
        params={},
        headers=admin_token_header
...
async def test_read_file_info_list_from_others_storage_as_admin(
    ...
    with image_file.open("rb") as f:
        await test_db.grid_client.upload_from_stream(filename=image_file.name, source=f, metadata={"user_id": "some_id"})
    with audio_file.open("rb") as f:
        await test_db.grid_client.upload_from_stream(filename=audio_file.name, source=f, metadata={"user_id": "some_other_id"})
    response = await test_client.get(
        "/api/files/list",
...
async def test_read_file_info_list_as_uploader(
    test_client: AsyncClient, test_db: MockDatabase, text_file: Path, image_file: Path, audio_file: Path, uploader_token_header: str
):
    with text_file.open("rb") as f:
        await test_db.grid_client.upload_from_stream(filename=text_file.name, source=f, metadata={"user_id": "uploader_id"})
    with image_file.open("rb") as f:
        await test_db.grid_client.upload_from_stream(filename=image_file.name, source=f, metadata={"user_id": "uploader_id"})
    with audio_file.open("rb") as f:
        await test_db.grid_client.upload_from_stream(filename=audio_file.name, source=f, metadata={"user_id": "uploader_id"})
    response = await test_client.get(
        "/api/files/list",
        params={},
        headers=uploader_token_header
...
async def test_read_file_info_list_as_viewer(
    test_client: AsyncClient, test_db: MockDatabase, text_file: Path, image_file: Path, audio_file: Path, viewer_token_header: str
):
    with text_file.open("rb") as f:
        await test_db.grid_client.upload_from_stream(filename=text_file.name, source=f, metadata={"user_id": "viewer_id"})
    with image_file.open("rb") as f:
        await test_db.grid_client.upload_from_stream(filename=image_file.name, source=f, metadata={"user_id": "viewer_id"})
    with audio_file.open("rb") as f:
        await test_db.grid_client.upload_from_stream(filename=audio_file.name, source=f, metadata={"user_id": "viewer_id"})
    response = await test_client.get(
        "/api/files/list",
        params={},
        headers=viewer_token_header
